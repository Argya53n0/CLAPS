...
@section('content')
    <div class="max-w-2xl bg-white rounded-2xl border border-[#EBE1D7] shadow-sm overflow-hidden">
        <form action="{{ route('admin.menu.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="category" required class="w-full px-4 py-3 bg-[#FCF8F2] border border-[#EBE1D7] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#8C4A15]/20 focus:border-[#8C4A15] transition-all text-[15px]">
                    ...
                    <option value="non_coffee" {{ old('category') == 'non_coffee' ? 'selected' : '' }}>Non-Coffee</option>
            ...
            <div>
                ...
                <textarea name="description" rows="3" 
                          class="w-full px-4 py-3 bg-[#FCF8F2] border border-[#EBE1D7] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#8C4A15]/20 focus:border-[#8C4A15] transition-all text-[15px]">{{ old('description') }}</textarea>
            ...
            <div class="pt-4 flex gap-4">
                ...
                <button type="submit" class="flex-1 bg-[#8C4A15] text-white font-bold rounded-xl hover:bg-[#723C10] transition-colors">Save Menu</button>
            </div>
        </form>
    </div>
@endsection
